...
export async function publishCommunityYoutubePlan(db, plan, { supabaseUrl, cronSecret }) {
  ...
  const yt = body?.results?.youtube;
  if (yt?.ok === true) {
    return { status: res.status, body: text, publishVariant, youtubeId: yt.id ?? null };
  }
  if (yt?.throttled) {
    throw new Error(yt.reason ?? yt.error ?? "youtube_throttled");
  }
  throw new Error(yt?.error ?? body?.error ?? "youtube_publish_failed");
}

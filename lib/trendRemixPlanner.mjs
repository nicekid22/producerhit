...
export const TREND_REMIX_SLOTS = ["morning", "evening"];

/** morning → remix1, evening → remix2 */
...
export async function seedTrendRemixPlansForDay(db, day = dayKey()) {
  ...
  const rows = missing.map((slot, i) => {
    const idx = hash(`${day}:${slot}`) % catalog.length;
    const entry = catalog[(idx + i) % catalog.length];
    return {
      day,
      slot,
      catalog_id: entry.id,
...
export { dayKey, SLOT_YOUTUBE_ACCOUNT, TREND_REMIX_SLOTS };

...
import { youtubeTrendRemixMaxSec } from "./youtubeLandscapeTemplate.mjs";
...
export async function renderAndUploadYouTubeVideo(db, loop) {
  ...
  const sec = isTrendRemix ? youtubeTrendRemixMaxSec() : isViral ? youtubeViralPreviewSec() : youtubePreviewSec();
  const aceLyrics =
  ...
  try {
    ...
    return {
      ...
      trendRemix: isTrendRemix,
      visual: isTrendRemix ? "landscape" : isViral ? "stock" : "cover",
    };
  } finally {

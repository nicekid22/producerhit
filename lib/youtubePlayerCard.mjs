...
export async function renderPlayerCardPng({
  ...
  const subText = truncate(subtitle || meta.subtitle, 40);
  ...
  const subY = PLAYER_CARD.subY;
  const barY = PLAYER_CARD.barY;
  const controlsY = PLAYER_CARD.controlsY;
  ...
  <text x="${artX}" y="46" fill="${meta.subColor}" font-family="Inter, Segoe UI, sans-serif" font-size="16" letter-spacing="2.5">${escXml(meta.header)}</text>
  ${appleEqBarsSvg(artX, artY, artSize, meta.eqBarFill)}
  <text x="${artX}" y="${titleY}" fill="${meta.accent}" font-family="${meta.titleFont}" font-size="${meta.titleSize}" font-weight="${meta.titleWeight}" font-style="${meta.titleStyle}">${escXml(titleText)}</text>

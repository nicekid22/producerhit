...
export async function repairYoutubeDailyPlans(db, day = null) {
  ...
  const { data: stuckRendering } = await db
    .from("youtube_daily_plans")
    .select("id")
    .eq("status", "rendering")
    .lt("updated_at", stuckBefore);
  for (const row of stuckRendering ?? []) {
    await markYoutubeDailyPlan(db, row.id, { status: "planned", last_error: "rendering_timeout_recovered" });
    unstuck += 1;
  }

  const { data: retryFailed } = await db
    .from("youtube_daily_plans")
    .select("id, last_error")
    .eq("status", "failed")
    .not("loop_id", "is", null);
  for (const row of retryFailed ?? []) {
    const err = String(row.last_error ?? "");
    if (err.includes("loop_not_ready") || err.includes("already_published") || err.includes("trend_remix_pipeline")) continue;
    await markYoutubeDailyPlan(db, row.id, { status: "planned", last_error: null });
    repaired += 1;
  }
  ...
  return { repaired, skipped, unstuck, days: days.length };
}
...
export async function getNextYoutubeDailyPlans(db, { limit = 3, day = null } = {}) {
  ...
  async function fetchByFormat(format) {
    const { data, error } = await db
      .from("youtube_daily_plans")
      .select("*, loops(*)")
      .eq("day", pendingDay)
  ...
  const shorts = await fetchByFormat("short");
  const plans = [...shorts];
  if (plans.length < limit) {
    const longs = await fetchByFormat("long");
    plans.push(...longs.slice(0, limit - plans.length));
  }

  return plans.slice(0, limit).map((row) => ({
    ...row,
    loop: row.loops ?? null,
  }));
}

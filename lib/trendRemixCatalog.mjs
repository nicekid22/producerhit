 ...
 * Lyrics: fill `lyrics` when available; otherwise ACE uses `lyrics_theme` (original writing).
 */
...
export function buildAceLyrics(entry) {
  ...
  if (provided.length > 40) return provided;
  return [
    `[Verse — original theme, do not copy copyrighted lyrics]`,
    entry.lyrics_theme,
    "",
    "[Chorus]",
    `Reimagine "${entry.original_title}" energy in ${entry.remix_genre} — new words, same emotion.`,
  ].join("\n");
}

...
const LANDSCAPE_CACHED = join(process.cwd(), "public", "img", "overlays", `film-dust-landscape-v${VERSION}.png`);
...
function patchDensity(x, y, rnd, frameW = W, frameH = H) {
  const nx = x / frameW;
  const ny = y / frameH;
...
function buildDustSvg(width = W, height = H, seedSuffix = "") {
  const rnd = seededRng(`producerhit-film-dust-v${VERSION}${seedSuffix}`);
  const dots = [];
  const dotCount = Math.round(1400 * (width / W) * (height / H));

  for (let i = 0; i < dotCount; i += 1) {
    const x = rnd() * width;
    const y = rnd() * height;
    const density = patchDensity(x, y, rnd, width, height);
  ...
  for (let i = 0; i < 7; i += 1) {
    const x1 = rnd() * width;
    const y1 = rnd() * height;
    const density = patchDensity(x1 / (width / W), y1 / (height / H), rnd);
...
async function writeDustPng(target, width, height, seedSuffix = "") {
  mkdirSync(dirname(target), { recursive: true });
  const svg = Buffer.from(buildDustSvg(width, height, seedSuffix));
...
export async function ensureFilmDustTexture(outPath) {
  ...
  return writeDustPng(outPath ?? CACHED, W, H);
}

/** Landscape 16:9 film dust — trend remix cinematic-glow. */
export async function ensureFilmDustLandscapeTexture(outPath) {
  if (existsSync(LANDSCAPE_CACHED)) return LANDSCAPE_CACHED;
  return writeDustPng(outPath ?? LANDSCAPE_CACHED, LANDSCAPE_W, LANDSCAPE_H, ":landscape");
}

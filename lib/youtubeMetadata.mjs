/** Node mirror of supabase/functions/_shared/youtubeMetadata.ts */

import { fillTemplate, pickAbVariant } from "./youtubeAbTesting.mjs";
import { getChannelProfile, homeUrlForChannel } from "./youtubeChannelProfiles.mjs";
...
function vars(input) {
  ...
  return {
    ...
    ep: input.ep ?? "",
    sourceShort: String(input.sourceShort ?? "").trim().slice(0, 40),
  };
}

function standardTitleVariants(accountId, kind) {
...
function viralTitleVariants(series) {
...
function buildStandardHook(kind, genre, name) {
...
function buildViralHook(viral, profileKeyword) {
...
function buildTags(input) {
  ...
  const genreSlug = String(input.genre ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "");
  const base = [
...
function buildHashtagLine(accountId, genre) {
...
export function buildYouTubeUploadMetadata(input) {
  const accountId = String(input.accountId ?? "vibez")
    .trim()
    .toLowerCase();
  const profile = getChannelProfile(accountId);
  ...
  const descCtx = {
    hookOpen: input.viralMeta
      ...
      : buildStandardHook(input.kind, input.genre, input.name),
    sourceQuote,
...
export function previewAllAbVariants(loopId, accountId, sample = {}) {
  const accounts = accountId ? [accountId] : ["vibez", "market", "lowdey", "producerhitai", "beatmakerunion"];
  const out = {};
  for (const acc of accounts) {
    const meta = buildYouTubeUploadMetadata({
      loopId,
      name: sample.name ?? "Midnight Drive",
      genre: sample.genre ?? "Trap",
      bpm: sample.bpm ?? 140,
      key: sample.key ?? "Am",
      kind: sample.kind ?? "type_beat",
      shareUrl: `https://www.producerhit.com/loop/${loopId}?utm_source=youtube`,
      accountId: acc,
      viralMeta: sample.viralMeta ?? null,
    });
    out[acc] = meta;
  }
  return out;

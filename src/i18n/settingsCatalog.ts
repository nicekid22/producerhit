...
const S = {
  ...
  navTagStudio: L({ en: "Tag Studio", fr: "Tag Studio", es: "Tag Studio", pt: "Tag Studio", de: "Tag Studio", it: "Tag Studio", nl: "Tag Studio", ar: "Tag Studio", ja: "Tag Studio", ko: "Tag Studio", tr: "Tag Studio", hi: "Tag Studio", zh: "Tag Studio", th: "Tag Studio" }),
  tagStudioDescription: L({ en: "Upload your producer tag / jingle (3–8 s). Apply it on any generated track from the Library.", fr: "Uploade ton tag producteur / jingle (3–8 s). Applique-le sur n'importe quel morceau généré depuis la Bibliothèque.", es: "Sube tu tag de productor / jingle (3–8 s). Aplícalo en cualquier pista generada desde la Biblioteca.", pt: "Faça upload do seu tag de produtor / jingle (3–8 s). Aplique em qualquer faixa gerada pela Biblioteca.", de: "Lade deinen Producer-Tag / Jingle (3–8 s) hoch. Wende ihn auf jeden generierten Track in der Bibliothek an.", it: "Carica il tuo tag di produttore / jingle (3–8 s). Applicalo su qualsiasi traccia generata dalla Libreria.", nl: "Upload je producent-tag / jingle (3–8 s). Pas het toe op elke gegenereerde track in de Bibliotheek.", ar: "ارفع علامة المنتج / جينجل (3–8 ث). طبقها على أي مسار مُنشأ من المكتبة.", ja: "プロデューサータグ / ジングル（3–8秒）をアップロード。ライブラリの生成済みトラックに適用。", ko: "프로듀서 태그 / 징글(3–8초)을 업로드하세요. 라이브러리의 생성된 트랙에 적용하세요.", tr: "Prodüktör etiketinizi / jinglenizi (3–8 sn) yükleyin. Kitaplıktaki herhangi bir oluşturulmuş parçaya uygulayın.", hi: "अपना प्रोड्यूसर टैग / जिंगल (3–8 सेकंड) अपलोड करें। लाइब्रेरी से किसी भी जनरेट किए गए ट्रैक पर लागू करें।", zh: "上传你的制作人标签/前奏（3–8秒）。应用到资料库中任何生成的曲目。", th: "อัปโหลดแท็กโปรดิวเซอร์/จิงเกิล (3–8 วินาที) แล้วใช้กับแทร็กที่สร้างจากไลบรารี" }),
  ...
  legalNameTitle: L({ en: "Legal name (optional)", fr: "Nom légal (optionnel)", es: "Nombre legal (opcional)", pt: "Nome legal (opcional)", de: "Rechtlicher Name (optional)", it: "Nome legale (opzionale)", nl: "Juridische naam (optioneel)", ar: "الاسم القانوني (اختياري)", ja: "法的氏名（任意）", ko: "법적 이름(선택)", tr: "Yasal ad (isteğe bağlı)", hi: "कानूनी नाम (वैकल्पिक)", zh: "法定姓名（可选）", th: "ชื่อตามกฎหมาย (ทางเลือก)" }),
  legalNameHint: L({ en: "Private — only if you need contract-ready PDFs. Certificates use your artist username by default.", fr: "Privé — seulement si tu veux des PDF prêts pour contrat. Les certificats utilisent ton pseudo artiste par défaut.", es: "Privado — solo si necesitas PDFs listos para contrato. Los certificados usan tu usuario artístico por defecto.", pt: "Privado — só se precisar de PDFs prontos para contrato. Certificados usam seu nome de artista por padrão.", de: "Privat — nur für vertragsfertige PDFs. Zertifikate nutzen standardmäßig deinen Künstlernamen.", it: "Privato — solo per PDF pronti per contratto. I certificati usano il tuo username artista di default.", nl: "Privé — alleen voor contract-PDF's. Certificaten gebruiken standaard je artiestennaam.", ar: "خاص — فقط إذا احتجت PDF جاهزة للعقد. الشهادات تستخدم اسم الفنان افتراضياً.", ja: "非公開 — 契約向けPDFが必要な場合のみ。証明書は既定でアーティスト名を使用。", ko: "비공개 — 계약용 PDF가 필요할 때만. 인증서는 기본적으로 아티스트 사용자명을 사용합니다.", tr: "Gizli — yalnızca sözleşmeye hazır PDF gerekiyorsa. Sertifikalar varsayılan olarak sanatçı adını kullanır.", hi: "निजी — केवल अनुबंध-तैयार PDF के लिए। प्रमाणपत्र डिफ़ॉल्ट रूप से कलाकार उपयोगकर्ता नाम का उपयोग करते हैं।", zh: "私密 — 仅在需要合同级 PDF 时填写。证书默认使用你的艺名。", th: "ส่วนตัว — เฉพาะเมื่อต้องการ PDF สำหรับสัญญา ใบรับรองใช้ชื่อศิลปินตามค่าเริ่มต้น" }),
...
export function buildSettingsSection(locale: AppLocale) {
  ...
  return {
    ...
    navTagStudio: t("navTagStudio"),
    tagStudioDescription: t("tagStudioDescription"),

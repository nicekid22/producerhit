...
import { suggestDistributionGenre, readLoopProducerTagMeta } from "@producerhit/shared";
...
export function DistributionWizard({
  ...
  const hasProducerTag = Boolean(readLoopProducerTagMeta(effectiveLoop.stemsUrl));
    ...
    >
      ...
      {step === 1 ? (
        <div>
          ...
          {hasProducerTag ? (
            <p className="mt-3 rounded-xl border border-violet-400/25 bg-violet-500/10 px-3 py-2 text-sm text-violet-100">
              {copy.producerTagPackNote}
            </p>
          ) : null}

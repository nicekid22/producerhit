  ...
  if (compact) {
    ...
    return (
      <div ref={ref} className="pk-landing-testimonials pk-landing-testimonials--compact">
        <LandingSectionHead
          eyebrow={copy.testimonialsTitle}
          title={copy.testimonialsHeadline}
        />
        <div className="pk-landing-testimonials__compact-grid mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((t) => (
            <blockquote key={t.id} className="pk-landing-testimonials__card pk-landing-apple-surface flex h-full flex-col p-5">
              <TestimonialStars />
              <Quote className="mt-2 h-5 w-5 shrink-0" aria-hidden />
              <p className="mt-3 flex-1 text-sm leading-relaxed">&ldquo;{t.q}&rdquo;</p>
              <footer className="mt-3 text-xs font-semibold">{t.who}</footer>
            </blockquote>
          ))}
        </div>
      </div>

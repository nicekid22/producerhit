...
import { runCheckoutWithAuth, planRank, type PaidPlan } from "@/lib/billing";
...
type Props = {
  ...
  currentPlan?: string;
  /** Attendre hydratation profil — évite flash upsell pour comptes payants */
  planReady?: boolean;
};
...
export function CheckoutRecoveryBanner({
  ...
  useEffect(() => {
    if (!planReady || !currentPlan || !isPaidPlan(plan)) return;
    if (planRank(currentPlan) >= planRank(plan)) {
      clearCheckoutAbandoned();
    }
  }, [currentPlan, plan, planReady]);

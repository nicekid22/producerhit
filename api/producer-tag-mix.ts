...
async function readBodyBuffer(req: Readable): Promise<Buffer> {
  const chunks: Buffer[] = [];
...
export default async function handler(req: MixReq, res: MixRes) {
  if (req.method !== "POST") {
  ...
  if (!contentType.includes("multipart/form-data")) {
    res.status(400).send(JSON.stringify({ error: "multipart_required" }));
    return;
  }
  ...
  if (!boundary) {
    res.status(400).send(JSON.stringify({ error: "invalid_boundary" }));
    return;
  }
  ...
  try {
    ...
    if (!Buffer.isBuffer(beat) || !Buffer.isBuffer(tag)) {
      res.status(400).send(JSON.stringify({ error: "beat_and_tag_required" }));
      return;
    }
    ...
    res.status(500).send(JSON.stringify({ error: e instanceof Error ? e.message : "mix_failed" }));
  } finally {

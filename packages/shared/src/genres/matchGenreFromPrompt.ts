import { ALL_GENRE_OPTIONS } from "./genreMenu";
import { pickRandomGenreValue } from "./genrePickMode";

const CATALOG_VALUES = new Set(ALL_GENRE_OPTIONS.map((o) => o.value));
...
function matchGenreByKeywordRules(hay: string): string | null {
  const rules: Array<{ test: (s: string) => boolean; genre: string }> = [
    ...
    {
      test: (s) => s.includes("afrobeats") || (s.includes("afro") && !s.includes("afro house") && !s.includes("afro r")),
      genre: "Afrobeats",
    },
...
export function matchGenreFromPrompt(prompt: string): string | null {
  ...
  const sorted = [...ALL_GENRE_OPTIONS].sort(

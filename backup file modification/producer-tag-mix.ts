...
export default async function handler(req: IncomingMessage & { method?: string; headers: Record<string, string | string[] | undefined> }, res: ServerResponse) {
  ...
  if (!boundary) {
    res.writeHead(400, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: "invalid_boundary" }));
    return;
  }
  ...
  try {
    ...
    if (!Buffer.isBuffer(beat) || !Buffer.isBuffer(tag)) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "beat_and_tag_required" }));
      return;
    }
    ...
    if (mode === "normalize_only") {
      ...
      res.writeHead(200, { "Content-Type": "audio/mpeg", "Content-Length": String(normBytes.byteLength) });
      res.end(normBytes);
      return;
    }
    ...
    res.writeHead(200, { "Content-Type": "audio/mpeg", "Content-Length": String(outBytes.byteLength) });
    res.end(outBytes);
  } catch (e) {
    console.error("producer-tag-mix", e);
    res.writeHead(500, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: e instanceof Error ? e.message : "mix_failed" }));
  } finally {
